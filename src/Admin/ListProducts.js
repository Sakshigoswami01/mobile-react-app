...
export default function ListProducts(props) {
  ...
  const [getProductImage, setProductImage] = useState({ url: "noimg.png", bytes: "" });
  ...
  const handleClickOpen = (rowData) => {
    ...
    setImage({ url: `${ServerURL}/images/${rowData.picture}`, bytes: "" });
    setProductImage({ url: `${ServerURL}/images/${rowData.productimage}`, bytes: "" });
    setOpen(true);
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!error) {
      ...
      formdata.append("productimage", getProductImage.bytes);
  ...
  const handleDelete = async () => {
    ...
    formdata.append("productid", getProductid);
    const config = { header: { "content-type": "multipart/form-data" } };
    var result = await postDataAndImage("product/delete", formdata, config);
  ...
  const fetchData = async () => {
    var list = await getData("product/listproducts");
    setlist(list.data);
  };
        ...
        >
          ...
          <div className={classes.root}>
            ...
            <div style={{
              ...
              <img src={`${getProductImage.url}`} width='100' height='100' /> 
            </div>
            <input
              accept="image/*"
              className={classes.input}
